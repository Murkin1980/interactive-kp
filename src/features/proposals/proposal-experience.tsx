    ...
    <main className="proposal-canvas min-h-screen overflow-hidden text-[#28251f]">
      ...
      <header className="relative z-10 flex items-center justify-between border-b border-[#39352e]/15 px-5 py-4 md:px-10">
        <div>
          ...
          <p className="text-[10px] uppercase tracking-[.28em] text-[#c2a46d]">Персональный проект</p>
        </div>
        <div className="flex items-center gap-4 text-xs text-[#746d62]">
          <span>01 <i className="mx-2 inline-block h-px w-8 bg-[#c2a46d] align-middle" /> 04</span>
      ...
      <section className="relative z-10 grid min-h-[68vh] border-b border-[#39352e]/15 lg:grid-cols-[1.08fr_.92fr]">
        ...
        <div className="flex flex-col justify-center px-6 py-12 md:px-12 lg:px-16">
          <p className="mb-4 text-xs font-semibold uppercase tracking-[.3em] text-[#702f35]">Ваш проект</p>
          <h1 className="max-w-2xl font-serif text-5xl leading-[.96] tracking-[-.035em] md:text-7xl">{proposal.project_name}</h1>
          <div className="my-8 h-px w-full bg-gradient-to-r from-[#c2a46d] via-[#c2a46d]/30 to-transparent" />
      ...
      {proposal.items.map((item, itemIndex) => {
        ...
        return <section key={item.id} className="relative z-10 border-b border-[#39352e]/15 px-5 py-12 md:px-10 lg:py-16">
          <div className="mb-7 flex items-end justify-between">
            <div><p className="text-[10px] uppercase tracking-[.28em] text-[#702f35]">Предмет {String(itemIndex + 1).padStart(2,"0")}</p><h2 className="mt-2 font-serif text-3xl md:text-5xl">{item.name}</h2></div>
            ...
            <ChoiceRail title="Материалы и исполнение">
              {item.variants.map((variant) => {
                ...
                return <button key={variant.id} type="button" disabled={props.confirmed || props.expired} onClick={() => props.onVariantChange(item.id,variant.id)} className={`choice-tile group ${active?"choice-tile-active":""}`}>
                  ...
                  <span className="text-sm font-semibold text-[#702f35]">{formatCurrency(variant.price)}</span>
                  {active && <Check className="absolute right-2 top-2 rounded-full bg-[#40233a] p-1 text-white" size={20}/>} 
                </button>;
              })}
            </ChoiceRail>
      ...
      <section className="relative z-10 grid border-b border-[#39352e]/15 lg:grid-cols-[1fr_420px]">
        <div className="px-6 py-14 md:px-10 lg:py-20">
          <p className="text-xs uppercase tracking-[.28em] text-[#702f35]">Финальный шаг</p>
        ...
        <aside className="bg-[#e6edef]/85 p-6 shadow-[-20px_0_60px_rgba(20,38,61,.10)] md:p-9">
          ...
          {!props.confirmed && !props.expired ? <form onSubmit={props.onConfirm} className="space-y-3">
            ...
            <label className="flex cursor-pointer gap-3 py-2 text-xs leading-5 text-[#536574]"><input type="checkbox" checked={props.consent} onChange={e=>props.onConsentChange(e.target.checked)} className="mt-1 accent-[#40233a]"/><span>Я проверил комплектацию и согласен с окончательной сметой.</span></label>
            {props.submitError&&<p className="text-xs text-red-700">{props.submitError}</p>}
            <button disabled={props.confirming} className="group flex w-full items-center justify-between bg-[#40233a] px-5 py-4 text-sm font-medium text-white transition-all duration-300 hover:bg-[#702f35] hover:shadow-xl disabled:opacity-60"><span>{props.confirming?"Формируем документы…":"Согласовать итоговую смету"}</span><ArrowRight className="transition-transform group-hover:translate-x-1" size={19}/></button>
